...
class LocalObservator(ObservationBuilder):
    '''
    ...
    Features:
        0. Cell type of the rail in the agent's Local
        1. Cell orientation of the rail in the agent's Local
        2. Distances in shortest path in the agent's Local
        3. Other agents positions in the agent's Local (direction of each agent)
        4. Agents targets in the agent's Local (1 agent target, 0 other agent, -1 otherwise)
        5. Agents malfunctioning turns
        6. Agents fractional speeds
        7+. Distances in deviation paths in the agent's Local
    '''

    def __init__(self, max_depth, predictor):
        super().__init__()
        # Always keep an odd number of "squares", so that the agent
        # is centered w.r.t. its Local
        assert max_depth % 2 != 0, 'Local window must be an odd number'
    ...
    def extract_path_Local(self, path, lenght, pad=0):
        '''
        Given a path returns the matrix Local marking the occupied positions assuming
        the first position as the center one of the Local
        '''
        path_Local = np.full((self.max_depth, self.max_depth), pad)
        distance = lenght
        if distance < np.inf:
    ...
    def get(self, handle=0):
        ...
        if (self.predictions[handle] is not None):
            ...
            if agent_position is not None:
                ...
                # Cell type of the rail in the agent's Local
                cell_type = utils.extract_Local(
                    self.rail_obs[:, :, 0], agent_position, self.max_depth, -1
                )

                # Cell orientation of the rail in the agent's Local
                cell_orientation = utils.extract_Local(
                    self.rail_obs[:, :, 1], agent_position, self.max_depth, -1
                )

                # Distances in shortest path in the agent's Local
                path_Local = self.extract_path_Local(
                    shortest_pred.positions, shortest_pred.lenght, pad=-1
                )

                # Distances in deviation paths in the agent's Local
                dev_paths_Local = np.full(
                    (num_devs, self.max_depth, self.max_depth), -1
                )
                for i, dev_pred in enumerate(deviations_pred):
                    dev_path_Local = np.full(
                    ...
                    if dev_pred.lenght < np.inf:
                        ...
                        if source_index != -1:
                            ...
                            dev_path_Local = self.extract_path_Local(
                                dev_pos, len(dev_pos), pad=-1
                            )
                    dev_paths_Local[i] = dev_path_Local

                # Other agents positions in the agent's Local (direction of each agent)
                agents_Local = utils.extract_Local(
                    self.agent_positions, agent_position, self.max_depth, -1
                )

                # Agents targets in the agent's Local (1 agent target, 0 other agent, -1 otherwise)
                targets_Local = utils.extract_Local(
                    self.agent_targets[handle, :, :],
                    agent_position, self.max_depth, -1
                )

                # Malfunctioning turns
                malf_Local = utils.extract_Local(
                    self.agent_malfunctions, agent_position, self.max_depth, -1
                )

                # Speed information
                speed_Local = utils.extract_Local(
                ...
                self.observations[handle][2] = path_Local
                self.observations[handle][3] = agents_Local
                self.observations[handle][4] = targets_Local
                self.observations[handle][5] = malf_Local
                self.observations[handle][6] = speed_Local
                self.observations[handle][7: 7 + num_devs + 1] = dev_paths_Local
